...
def maybe_download_and_extract():
  ...
  如果我们所使用的预训练的模型不存在，那这个函数的作用就是下载这个模型,然后再解压，目录默认为./inception
  """
    # ./inception
    dest_directory = FLAGS.model_dir
    if not os.path.exists(dest_directory):
        os.makedirs(dest_directory)
    # 文件名：inception-2015-12-05.tgz
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  """
    ...
    # tf.name_scope将不同的对象及操作放在由tf.name_scope()指定的区域中，便于在tensorboard中展示清晰的逻辑关系图
...
def main(_):
    ...
    # 下载预训练的inception模型再解压
    maybe_download_and_extract()
    graph, bottleneck_tensor, jpeg_data_tensor, resized_image_tensor = (
        create_inception_graph())

    # Look at the folder structure, and create lists of all the images.
    # testing_percentage：多少百分比的图像用于测试集，default:10
    image_lists = create_image_lists(FLAGS.image_dir, FLAGS.testing_percentage,
                                     FLAGS.validation_percentage)
    class_count = len(image_lists.keys())
    # class_count:类别数，比如四种垃圾种类，就返回4
    if class_count == 0:
    ...
    with tf.Session(graph=graph) as sess:
        ...
        # 输入：bottleneck_tensor（cnn output）
        # Returns:
        #     The tensors for the training and cross entropy results, and tensors for the
        #     bottleneck input and ground truth input.
        # ground truth：简单来说就是有效的正确的数据（还不太理解）
